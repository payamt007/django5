...
class FeedViewSet(mixins.ListModelMixin, mixins.CreateModelMixin, mixins.UpdateModelMixin, viewsets.GenericViewSet):

    """
    Viewset for handling creating, listing and updating (unfollowing, following) rss feeds
    """
    ...
    @extend_schema(description='Update a Feed , followed to true or false , Or maybe change title or url')
...
class PostFilterAPIView(APIView):
    ...
    def get(self, request):
        ...
        filterd_results = Post.objects.filter(
            **serializer.validated_data).order_by(order_by_param)
...
class ForceRefreshAPIview(APIView):
    ...
    def post(self, request) -> Response:
        ...
        try:
            ...
            feed.stopped = False
            feed.save()
            return Response(data={"message": "Feed refresh Success! Feed returned to normal scrapping schedule!"},
                            status=status.HTTP_200_OK)
        except Exception:
            return Response(data={"message": "Feed refresh Faild! Sorry!"}, status=status.HTTP_406_NOT_ACCEPTABLE)
